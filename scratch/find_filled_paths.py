...
for f in sorted(os.listdir(new_svg_dir)):
    ...
    for idx, p in enumerate(paths):
        ...
        stroke = p.attrib.get('stroke', '').lower()
        if (fill and fill != 'none' and fill != '#ffffff') or (stroke and stroke != 'none' and stroke != '#000000'):
            pts = parse_points(p.attrib.get('d', ''))
            if not pts:
                continue
            ...
            print(f"Path #{idx}: fill='{fill}', stroke='{stroke}', bbox=[{xmin:.1f}, {ymin:.1f}] to [{xmax:.1f}, {ymax:.1f}], pts={len(pts)}")
